...
def train_case(args, case_id: int, device: torch.device):
    ...
    while iteration < args.max_iterations:
        ...
        if epoch == 1 or iteration >= args.max_iterations or epoch % args.log_every == 0:
            ...
            log_rows.append(row)
            print(
                f"PRes-MHSA-SBiLSTM case={case_id} epoch={epoch} iter={iteration} "
...
def write_results(summary):
    lines = [
        "PRes-MHSA-SBiLSTM VB regression experiment",
        "==========================================",
        "",
...
def parse_args():
    parser = argparse.ArgumentParser(description="PRes-MHSA-SBiLSTM VB regression experiment.")
